...
class Order extends MY_Controller
{
    ...
    public function data_menu_list()
    {
        ...
        foreach ($List as $r) {
            ...
            $row     = array();
            $menu_id = $r->menu_id;
            $row[]   = '<a title="Pilih Menu" class="pilihData"  data-id="' . $menu_id . '" data-code="' . $r->menu_kode . '" data-name="' . trim($r->menu_nama) . '" data-harga="' . $r->menu_harga . '"><i class="icon-check"></i></a>';
            $row[]   = $no;
            $row[]   = $r->menu_kode;
            $row[]   = $r->menu_nama;
            $row[]   = $r->kategori_nama;
